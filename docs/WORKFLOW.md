...
- **Internal Call-Trace Tree**
  - `main(...)`: command router and execution coordinator [`src/usereq/cli.py`]
    ...
    - `run(...)`: initialization/update command path for resources and config persistence; requires at least one of `--install-prompts`, `--install-agents`, `--install-skills` in non-update mode [`src/usereq/cli.py`]
