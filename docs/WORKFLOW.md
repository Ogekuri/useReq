...
- **Internal Call-Trace Tree**
  - `main(...)`: command router and execution coordinator [`src/usereq/cli.py`]
    - `maybe_notify_newer_version(timeout_seconds=RELEASE_CHECK_TIMEOUT_SECONDS)`: perform startup idle-gated online release-check before argument parsing and validation with lower-bound 300-second cadence and 86400-second post-success idle window [`src/usereq/cli.py`]
      ...
      - `should_execute_release_check(...)`: decide whether remote check is due using `last_success_timestamp + min_interval_seconds` and idle-until timestamp [`src/usereq/cli.py`]
