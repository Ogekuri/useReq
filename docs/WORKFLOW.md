...
- **Internal Call-Trace Tree**
  - `main(...)`: command router and execution coordinator [`src/usereq/cli.py`]
    - `maybe_notify_newer_version(timeout_seconds=RELEASE_CHECK_TIMEOUT_SECONDS)`: perform startup online release-check before argument parsing and validation; `main(...)` enables `FORCE_ONLINE_RELEASE_CHECK` for `--ver`/`--version` to bypass idle-gating; successful HTTP validation persists a fixed 300-second idle-delay and HTTP 429 or HTTP 403 API rate-limit failures persist a fixed 3600-second idle-delay [`src/usereq/cli.py`]
      ...
      - `write_release_check_idle_state(...)`: persist successful-check timestamp and idle-until timestamp using fixed 300-second delay after successful HTTP/JSON validation [`src/usereq/cli.py`]
      ...
      - `write_rate_limited_release_check_idle_state(...)`: persist idle-state on HTTP 429 or HTTP 403 API rate-limit failure using fixed 3600-second delay while preserving any larger existing idle-until timestamp [`src/usereq/cli.py`]
        - `write_release_check_idle_state_payload(...)`: serialize canonical idle-state JSON keys [`src/usereq/cli.py`]
          - `format_unix_timestamp_utc(...)`: serialize Unix timestamps as UTC human-readable strings [`src/usereq/cli.py`]
