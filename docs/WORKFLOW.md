  ...
  - `main(...)`: command router and execution coordinator [`src/usereq/cli.py`]
    - `maybe_notify_newer_version(timeout_seconds=RELEASE_CHECK_TIMEOUT_SECONDS)`: perform startup idle-gated online release-check before argument parsing and validation [`src/usereq/cli.py`]
      ...
      - `resolve_latest_release_api_url()`: resolve `https://api.github.com/repos/<owner>/<repository>/releases/latest` from active git remotes [`src/usereq/cli.py`]
        - `resolve_github_owner_repository_from_active_remotes()`: prioritize active fetch remotes, retry inspection from repository root on non-root git-context failure, and extract GitHub owner/repository [`src/usereq/cli.py`]
          - `read_git_remote_verbose(...)`: execute `git remote -v` in current or explicit working directory [`src/usereq/cli.py`]
    ...
    - `run_upgrade()`: dispatch uv-based self-upgrade before argparse parsing when `--upgrade` is present [`src/usereq/cli.py`]
      - `resolve_github_owner_repository_from_active_remotes()`: determine upgrade source `git+https://github.com/<owner>/<repository>.git` from remotes with repository-root retry on initial git-context failure [`src/usereq/cli.py`]
    ...
    - `run(...)`: initialization/update command path for resources and config persistence; requires at least one of `--install-prompts`, `--install-agents`, `--install-skills`, or `--provider` spec in non-update mode [`src/usereq/cli.py`]
      - `load_persisted_update_flags(...)`: merge persisted update/install flags from `.req/config.json` (`install-prompts`, `install-agents`, `install-skills`, providers, and toggles) [`src/usereq/cli.py`]
      ...
      - `resolve_provider_configs(...)`: merge global flags with `--provider` specs into per-provider config dicts keyed by provider name (SRS-275, SRS-277) [`src/usereq/cli.py`]
      ...
      - `build_persisted_update_flags(...)`: collect persisted boolean flags including install artifact controls (`install-prompts`, `install-agents`, `install-skills`) [`src/usereq/cli.py`]
