...
# cli.py | Python | 4666L | 149 symbols | 31 imports | 230 comments
...
- var `REPO_ROOT = Path(__file__).resolve().parents[2]` (L28)
- var `RESOURCE_ROOT = Path(__file__).resolve().parent / "resources"` (L31)
- @brief The absolute path to the repository root."""
- var `VERBOSE = False` (L34)
- @brief The absolute path to the resources directory."""
- var `DEBUG = False` (L37)
- @brief Whether verbose output is enabled."""
- var `REQUIREMENTS_TEMPLATE_NAME = "Requirements_Template.md"` (L40)
- @brief Whether debug output is enabled."""
- var `PERSISTED_UPDATE_FLAG_KEYS = ("preserve-models",)` (L43)
- @brief Name of the packaged requirements template file."""
- var `VALID_PROVIDERS = frozenset({"codex", "claude", "gemini", "github", "kiro", "opencode"})` (L46)
- @brief Config keys persisted for install/update boolean flags (SRS-288)."""
- var `VALID_ARTIFACTS = frozenset({"prompts", "agents"})` (L49)
- @brief Valid provider names accepted by ``--provider`` specs (SRS-275)."""
- var `VALID_PROVIDER_OPTIONS = frozenset(` (L52)
- @brief Valid artifact type tokens accepted in ``--provider`` specs (SRS-275)."""
- var `INVALID_WT_NAME_RE = re.compile(r'[<>:"/\\|?*\x00-\x1f\s]')` (L67)
- @brief Provider-to-directory mapping for worktree copy operations (SRS-325)."""
### fn `def parse_provider_spec(spec: str) -> tuple[str, set[str], set[str]]` (L71-122)
...
### fn `def resolve_provider_configs(` (L123-124)

- var `ANSI_BRIGHT_RED = "\033[91m"` (L172)
...
- fn `def __init__(self, message: str, code: int = 1) -> None` `priv` (L219-231)
  - @brief Dedicated exception for expected CLI errors.
...
### fn `def log(msg: str) -> None` (L232-241)
...
### fn `def dlog(msg: str) -> None` (L242-252)
...
### fn `def vlog(msg: str) -> None` (L253-263)
...
### fn `def _get_available_tags_help() -> str` `priv` (L264-276)
...
### fn `def build_parser() -> argparse.ArgumentParser` (L277-476)
...
### fn `def parse_args(argv: Optional[list[str]] = None) -> Namespace` (L540-549)
...
### fn `def load_package_version() -> str` (L550-564)
...
### fn `def maybe_print_version(argv: list[str]) -> bool` (L565-577)
...
### fn `def run_upgrade() -> None` (L578-604)
...
### fn `def run_uninstall() -> None` (L605-628)
...
### fn `def normalize_release_tag(tag: str) -> str` (L629-641)
...
### fn `def parse_version_tuple(version: str) -> tuple[int, ...] | None` (L642-666)
...
### fn `def is_newer_version(current: str, latest: str) -> bool` (L667-685)
...
### fn `def parse_github_owner_repository(remote_url: str) -> tuple[str, str] | None` (L686-712)
...
### fn `def read_git_remote_verbose(cwd: str | None = None) -> str` (L713-732)
...
### fn `def resolve_github_owner_repository_from_active_remotes() -> tuple[str, str]` (L733-793)
...
### fn `def resolve_latest_release_api_url() -> str` (L794-802)
...
### fn `def format_unix_timestamp_utc(timestamp_seconds: int) -> str` (L803-815)
...
### fn `def get_release_check_idle_file_path(` (L816-817)

### fn `def read_release_check_idle_state(file_path: Path) -> dict[str, int | str] | None` (L829-893)
...
### fn `def should_execute_release_check(` (L894-896)

### fn `def parse_retry_after_seconds(` (L914-916)
...
### fn `def write_release_check_idle_state_payload(` (L946-949)
...
### fn `def write_release_check_idle_state(` (L975-978)
...
### fn `def write_rate_limited_release_check_idle_state(` (L996-1001)
...
### fn `def maybe_notify_newer_version(` (L1035-1036)
...
### fn `def ensure_doc_directory(path: str, project_base: Path) -> None` (L1167-1189)
...
### fn `def ensure_test_directory(path: str, project_base: Path) -> None` (L1190-1212)
...
### fn `def ensure_src_directory(path: str, project_base: Path) -> None` (L1213-1235)
...
### fn `def make_relative_if_contains_project(path_value: str, project_base: Path) -> str` (L1236-1277)
...
### fn `def resolve_absolute(normalized: str, project_base: Path) -> Optional[Path]` (L1278-1293)
...
### fn `def format_substituted_path(value: str) -> str` (L1294-1305)
...
### fn `def compute_sub_path(` (L1306-1307)

### fn `def resolve_git_root(target_path: Path) -> Path` (L1328-1355)
...
### fn `def is_inside_git_repo(target_path: Path) -> bool` (L1356-1375)
...
### fn `def sanitize_branch_name(branch: str) -> str` (L1376-1385)
...
### fn `def validate_wt_name(wt_name: str) -> bool` (L1386-1397)
...
### fn `def load_full_config(project_base: Path) -> dict` (L1398-1417)
...
### fn `def save_config(` (L1418-1428)

### fn `def load_config(project_base: Path) -> dict[str, str | list[str]]` (L1472-1522)
...
### fn `def load_static_check_from_config(project_base: Path) -> dict` (L1523-1554)
...
### fn `def build_persisted_update_flags(args: Namespace) -> dict[str, bool]` (L1555-1568)
...
### fn `def load_persisted_update_flags(project_base: Path) -> dict[str, bool]` (L1569-1610)
...
### fn `def load_persisted_provider_specs(project_base: Path) -> list[str]` (L1611-1632)
...
### fn `def generate_guidelines_file_list(guidelines_dir: Path, project_base: Path) -> str` (L1633-1665)
...
### fn `def generate_guidelines_file_items(` (L1666-1667)

### fn `def upgrade_guidelines_templates(guidelines_dest: Path, overwrite: bool = False) -> int` (L1701-1735)
...
### fn `def make_relative_token(raw: str, keep_trailing: bool = False) -> str` (L1736-1752)
...
### fn `def ensure_relative(value: str, name: str, code: int) -> None` (L1753-1768)
...
### fn `def apply_replacements(text: str, replacements: Mapping[str, str]) -> str` (L1769-1781)
...
### fn `def write_text_file(dst: Path, text: str) -> None` (L1782-1793)
...
### fn `def copy_with_replacements(` (L1794-1795)

### fn `def normalize_description(value: str) -> str` (L1810-1824)
...
### fn `def md_to_toml(md_path: Path, toml_path: Path, force: bool) -> None` (L1825-1859)
...
### fn `def extract_frontmatter(content: str) -> tuple[str, str]` (L1860-1873)
...
### fn `def extract_description(frontmatter: str) -> str` (L1874-1886)
...
### fn `def extract_argument_hint(frontmatter: str) -> str` (L1887-1899)
...
### fn `def extract_purpose_first_bullet(body: str) -> str` (L1900-1924)
...
### fn `def _extract_section_text(body: str, section_name: str) -> str` `priv` (L1925-1952)
...
### fn `def json_escape(value: str) -> str` (L1953-1962)
...
### fn `def generate_kiro_resources(` (L1963-1966)

### fn `def render_kiro_agent(` (L1992-2001)
...
### fn `def replace_tokens(path: Path, replacements: Mapping[str, str]) -> None` (L2047-2060)
...
### fn `def yaml_double_quote_escape(value: str) -> str` (L2061-2070)
...
### fn `def list_docs_templates() -> list[Path]` (L2071-2090)
...
### fn `def find_requirements_template(docs_templates: list[Path]) -> Path` (L2091-2107)
...
### fn `def load_kiro_template() -> tuple[str, dict[str, Any]]` (L2108-2147)
...
### fn `def strip_json_comments(text: str) -> str` (L2148-2172)
...
### fn `def load_settings(path: Path) -> dict[str, Any]` (L2173-2188)
...
### fn `def load_centralized_models(` (L2189-2192)

### fn `def get_model_tools_for_prompt(` (L2247-2248)
...
### fn `def get_raw_tools_for_prompt(config: dict[str, Any] | None, prompt_name: str) -> Any` (L2288-2309)
...
### fn `def format_tools_inline_list(tools: list[str]) -> str` (L2310-2321)
...
### fn `def deep_merge_dict(base: dict[str, Any], incoming: dict[str, Any]) -> dict[str, Any]` (L2322-2337)
...
### fn `def find_vscode_settings_source() -> Optional[Path]` (L2338-2349)
...
### fn `def build_prompt_recommendations(prompts_dir: Path) -> dict[str, bool]` (L2350-2364)
...
### fn `def ensure_wrapped(target: Path, project_base: Path, code: int) -> None` (L2365-2380)
...
### fn `def save_vscode_backup(req_root: Path, settings_path: Path) -> None` (L2381-2395)
...
### fn `def restore_vscode_settings(project_base: Path) -> None` (L2396-2411)
...
### fn `def prune_empty_dirs(root: Path) -> None` (L2412-2429)
...
### fn `def remove_generated_resources(project_base: Path) -> None` (L2430-2474)
...
### fn `def run_remove(args: Namespace) -> None` (L2475-2524)
...
### fn `def _validate_enable_static_check_command_executables(` `priv` (L2525-2528)

### fn `def run(args: Namespace) -> None` (L2557-2756)
...
- var `VERBOSE = args.verbose` (L2566)
- @brief Handles the main initialization flow.
- @details Validates input arguments, normalizes paths, and orchestrates resource generation per provider and artifact type. Requires at least one ``--provider`` spec (SRS-035). Deduplicates ``--enable-static-check`` entries (SRS-251, SRS-301).
- @param args Parsed CLI namespace; must contain ``provider_specs`` list and ``preserve_models`` boolean.
- @return {None} Function return value.
- @satisfies SRS-251, SRS-301
- var `DEBUG = args.debug` (L2567)
- var `PROMPT = prompt_path.stem` (L3026)
### fn `def _format_install_table(` `priv` (L3452-3454)

### fn `def _wrap_cell(value: str, width: int, allow_wrap: bool) -> list[str]` `priv` (L3491-3513)
...
### fn `def _render_row(provider: str, prompts: str, modules: str) -> list[str]` `priv` (L3514-3542)
...
### fn `def _build_provider_modules_map(provider_specs: list[str]) -> dict[str, list[str]]` `priv` (L3563-3602)
...
### fn `def _colorize_table_border(line: str) -> str` `priv` (L3603-3615)
...
- var `SUPPORTED_EXTENSIONS = frozenset(` (L3630)
### fn `def _collect_source_files(src_dirs: list[str], project_base: Path) -> list[str]` `priv` (L3658-3715)
...
### fn `def _build_ascii_tree(paths: list[str]) -> str` `priv` (L3716-3761)
...
### fn `def _emit(` `priv` (L3740-3742)
- @brief Build a deterministic tree string from project-relative paths.
- @details Implements the _build_ascii_tree function behavior with deterministic control flow.
- @param paths Project-relative file paths.
- @return Rendered tree rooted at '.'.

### fn `def _format_files_structure_markdown(files: list[str], project_base: Path) -> str` `priv` (L3762-3776)
...
### fn `def _is_standalone_command(args: Namespace) -> bool` `priv` (L3777-3795)
...
### fn `def _is_project_scan_command(args: Namespace) -> bool` `priv` (L3796-3819)
...
### fn `def _is_here_only_project_scan_command(args: Namespace) -> bool` `priv` (L3820-3844)
...
### fn `def run_git_check(args: Namespace) -> None` (L3845-3876)
...
### fn `def run_docs_check(args: Namespace) -> None` (L3877-3906)
...
### fn `def run_git_wt_name(args: Namespace) -> None` (L3907-3935)
- @brief Execute --git-wt-name: print standardized worktree name.
- @param args Parsed CLI namespace.
- @return {None} Function return value.
- @satisfies SRS-318, SRS-319

### fn `def _worktree_path_exists_exact(git_path: Path, target_path: Path) -> bool` `priv` (L3936-3965)
...
### fn `def _rollback_worktree_create(git_path: Path, wt_path: Path, wt_name: str) -> None` `priv` (L3966-4002)
...
### fn `def run_git_wt_create(args: Namespace) -> None` (L4003-4094)
...
### fn `def run_git_wt_delete(args: Namespace) -> None` (L4095-4172)
...
### fn `def run_git_wt_exit(args: Namespace) -> None` (L4173-4189)
...
### fn `def run_files_tokens(files: list[str]) -> None` (L4190-4212)
...
### fn `def run_files_references(files: list[str]) -> None` (L4213-4229)
...
### fn `def run_files_compress(files: list[str], enable_line_numbers: bool = False) -> None` (L4230-4248)
...
### fn `def run_files_find(args_list: list[str], enable_line_numbers: bool = False) -> None` (L4249-4277)
...
### fn `def run_references(args: Namespace) -> None` (L4278-4295)
...
### fn `def run_compress_cmd(args: Namespace) -> None` (L4296-4317)
...
### fn `def run_find(args: Namespace) -> None` (L4318-4347)
...
### fn `def run_tokens(args: Namespace) -> None` (L4348-4375)
...
### fn `def run_files_static_check_cmd(files: list[str], args: Namespace) -> int` (L4376-4445)
...
### fn `def run_project_static_check_cmd(args: Namespace) -> int` (L4446-4491)
...
### fn `def _resolve_project_base(args: Namespace) -> Path` `priv` (L4492-4512)
...
### fn `def _resolve_project_src_dirs(args: Namespace) -> tuple[Path, list[str]]` `priv` (L4513-4565)
...
### fn `def main(argv: Optional[list[str]] = None) -> int` (L4566-4666)
...
- var `VERBOSE = getattr(args, "verbose", False)` (L4590)
- @brief CLI entry point for console_scripts and `-m` execution.
- @details Returns an exit code (0 success, non-zero on error).
- @param argv Input parameter `argv`.
- @return {int} Function return value.
- var `DEBUG = getattr(args, "debug", False)` (L4591)
## Symbol Index
|Symbol|Kind|Vis|Lines|Sig|
|---|---|---|---|---|
|`REPO_ROOT`|var|pub|28||
|`RESOURCE_ROOT`|var|pub|31||
|`VERBOSE`|var|pub|34||
|`DEBUG`|var|pub|37||
|`REQUIREMENTS_TEMPLATE_NAME`|var|pub|40||
|`PERSISTED_UPDATE_FLAG_KEYS`|var|pub|43||
|`VALID_PROVIDERS`|var|pub|46||
|`VALID_ARTIFACTS`|var|pub|49||
|`VALID_PROVIDER_OPTIONS`|var|pub|52||
|`INVALID_WT_NAME_RE`|var|pub|67||
|`parse_provider_spec`|fn|pub|71-122|def parse_provider_spec(spec: str) -> tuple[str, set[str]...|
|`resolve_provider_configs`|fn|pub|123-124|def resolve_provider_configs(|
|`ANSI_BRIGHT_RED`|var|pub|172||
|`ANSI_BRIGHT_GREEN`|var|pub|175||
|`ANSI_RESET`|var|pub|178||
|`RELEASE_CHECK_TIMEOUT_SECONDS`|var|pub|181||
|`RELEASE_CHECK_IDLE_DELAY_SECONDS`|var|pub|184||
|`TOOL_PROGRAM_NAME`|var|pub|187||
|`RELEASE_CHECK_PROGRAM_NAME`|var|pub|190||
|`GITHUB_REPOSITORY_OWNER`|var|pub|193||
|`GITHUB_REPOSITORY_NAME`|var|pub|196||
|`RELEASE_CHECK_IDLE_FILENAME_TEMPLATE`|var|pub|199||
|`GITHUB_RELEASES_LATEST_URL`|var|pub|202||
|`GITHUB_UPGRADE_SOURCE`|var|pub|208||
|`ReqError`|class|pub|214-231|class ReqError(Exception)|
|`ReqError.__init__`|fn|priv|219-231|def __init__(self, message: str, code: int = 1) -> None|
|`log`|fn|pub|232-241|def log(msg: str) -> None|
|`dlog`|fn|pub|242-252|def dlog(msg: str) -> None|
|`vlog`|fn|pub|253-263|def vlog(msg: str) -> None|
|`_get_available_tags_help`|fn|priv|264-276|def _get_available_tags_help() -> str|
|`build_parser`|fn|pub|277-476|def build_parser() -> argparse.ArgumentParser|
|`parse_args`|fn|pub|540-549|def parse_args(argv: Optional[list[str]] = None) -> Names...|
|`load_package_version`|fn|pub|550-564|def load_package_version() -> str|
|`maybe_print_version`|fn|pub|565-577|def maybe_print_version(argv: list[str]) -> bool|
|`run_upgrade`|fn|pub|578-604|def run_upgrade() -> None|
|`run_uninstall`|fn|pub|605-628|def run_uninstall() -> None|
|`normalize_release_tag`|fn|pub|629-641|def normalize_release_tag(tag: str) -> str|
|`parse_version_tuple`|fn|pub|642-666|def parse_version_tuple(version: str) -> tuple[int, ...] ...|
|`is_newer_version`|fn|pub|667-685|def is_newer_version(current: str, latest: str) -> bool|
|`parse_github_owner_repository`|fn|pub|686-712|def parse_github_owner_repository(remote_url: str) -> tup...|
|`read_git_remote_verbose`|fn|pub|713-732|def read_git_remote_verbose(cwd: str | None = None) -> str|
|`resolve_github_owner_repository_from_active_remotes`|fn|pub|733-793|def resolve_github_owner_repository_from_active_remotes()...|
|`resolve_latest_release_api_url`|fn|pub|794-802|def resolve_latest_release_api_url() -> str|
|`format_unix_timestamp_utc`|fn|pub|803-815|def format_unix_timestamp_utc(timestamp_seconds: int) -> str|
|`get_release_check_idle_file_path`|fn|pub|816-817|def get_release_check_idle_file_path(|
|`read_release_check_idle_state`|fn|pub|829-893|def read_release_check_idle_state(file_path: Path) -> dic...|
|`should_execute_release_check`|fn|pub|894-896|def should_execute_release_check(|
|`parse_retry_after_seconds`|fn|pub|914-916|def parse_retry_after_seconds(|
|`write_release_check_idle_state_payload`|fn|pub|946-949|def write_release_check_idle_state_payload(|
|`write_release_check_idle_state`|fn|pub|975-978|def write_release_check_idle_state(|
|`write_rate_limited_release_check_idle_state`|fn|pub|996-1001|def write_rate_limited_release_check_idle_state(|
|`maybe_notify_newer_version`|fn|pub|1035-1036|def maybe_notify_newer_version(|
|`ensure_doc_directory`|fn|pub|1167-1189|def ensure_doc_directory(path: str, project_base: Path) -...|
|`ensure_test_directory`|fn|pub|1190-1212|def ensure_test_directory(path: str, project_base: Path) ...|
|`ensure_src_directory`|fn|pub|1213-1235|def ensure_src_directory(path: str, project_base: Path) -...|
|`make_relative_if_contains_project`|fn|pub|1236-1277|def make_relative_if_contains_project(path_value: str, pr...|
|`resolve_absolute`|fn|pub|1278-1293|def resolve_absolute(normalized: str, project_base: Path)...|
|`format_substituted_path`|fn|pub|1294-1305|def format_substituted_path(value: str) -> str|
|`compute_sub_path`|fn|pub|1306-1307|def compute_sub_path(|
|`resolve_git_root`|fn|pub|1328-1355|def resolve_git_root(target_path: Path) -> Path|
|`is_inside_git_repo`|fn|pub|1356-1375|def is_inside_git_repo(target_path: Path) -> bool|
|`sanitize_branch_name`|fn|pub|1376-1385|def sanitize_branch_name(branch: str) -> str|
|`validate_wt_name`|fn|pub|1386-1397|def validate_wt_name(wt_name: str) -> bool|
|`load_full_config`|fn|pub|1398-1417|def load_full_config(project_base: Path) -> dict|
|`save_config`|fn|pub|1418-1428|def save_config(|
|`load_config`|fn|pub|1472-1522|def load_config(project_base: Path) -> dict[str, str | li...|
|`load_static_check_from_config`|fn|pub|1523-1554|def load_static_check_from_config(project_base: Path) -> ...|
|`build_persisted_update_flags`|fn|pub|1555-1568|def build_persisted_update_flags(args: Namespace) -> dict...|
|`load_persisted_update_flags`|fn|pub|1569-1610|def load_persisted_update_flags(project_base: Path) -> di...|
|`load_persisted_provider_specs`|fn|pub|1611-1632|def load_persisted_provider_specs(project_base: Path) -> ...|
|`generate_guidelines_file_list`|fn|pub|1633-1665|def generate_guidelines_file_list(guidelines_dir: Path, p...|
|`generate_guidelines_file_items`|fn|pub|1666-1667|def generate_guidelines_file_items(|
|`upgrade_guidelines_templates`|fn|pub|1701-1735|def upgrade_guidelines_templates(guidelines_dest: Path, o...|
|`make_relative_token`|fn|pub|1736-1752|def make_relative_token(raw: str, keep_trailing: bool = F...|
|`ensure_relative`|fn|pub|1753-1768|def ensure_relative(value: str, name: str, code: int) -> ...|
|`apply_replacements`|fn|pub|1769-1781|def apply_replacements(text: str, replacements: Mapping[s...|
|`write_text_file`|fn|pub|1782-1793|def write_text_file(dst: Path, text: str) -> None|
|`copy_with_replacements`|fn|pub|1794-1795|def copy_with_replacements(|
|`normalize_description`|fn|pub|1810-1824|def normalize_description(value: str) -> str|
|`md_to_toml`|fn|pub|1825-1859|def md_to_toml(md_path: Path, toml_path: Path, force: boo...|
|`extract_frontmatter`|fn|pub|1860-1873|def extract_frontmatter(content: str) -> tuple[str, str]|
|`extract_description`|fn|pub|1874-1886|def extract_description(frontmatter: str) -> str|
|`extract_argument_hint`|fn|pub|1887-1899|def extract_argument_hint(frontmatter: str) -> str|
|`extract_purpose_first_bullet`|fn|pub|1900-1924|def extract_purpose_first_bullet(body: str) -> str|
|`_extract_section_text`|fn|priv|1925-1952|def _extract_section_text(body: str, section_name: str) -...|
|`json_escape`|fn|pub|1953-1962|def json_escape(value: str) -> str|
|`generate_kiro_resources`|fn|pub|1963-1966|def generate_kiro_resources(|
|`render_kiro_agent`|fn|pub|1992-2001|def render_kiro_agent(|
|`replace_tokens`|fn|pub|2047-2060|def replace_tokens(path: Path, replacements: Mapping[str,...|
|`yaml_double_quote_escape`|fn|pub|2061-2070|def yaml_double_quote_escape(value: str) -> str|
|`list_docs_templates`|fn|pub|2071-2090|def list_docs_templates() -> list[Path]|
|`find_requirements_template`|fn|pub|2091-2107|def find_requirements_template(docs_templates: list[Path]...|
|`load_kiro_template`|fn|pub|2108-2147|def load_kiro_template() -> tuple[str, dict[str, Any]]|
|`strip_json_comments`|fn|pub|2148-2172|def strip_json_comments(text: str) -> str|
|`load_settings`|fn|pub|2173-2188|def load_settings(path: Path) -> dict[str, Any]|
|`load_centralized_models`|fn|pub|2189-2192|def load_centralized_models(|
|`get_model_tools_for_prompt`|fn|pub|2247-2248|def get_model_tools_for_prompt(|
|`get_raw_tools_for_prompt`|fn|pub|2288-2309|def get_raw_tools_for_prompt(config: dict[str, Any] | Non...|
|`format_tools_inline_list`|fn|pub|2310-2321|def format_tools_inline_list(tools: list[str]) -> str|
|`deep_merge_dict`|fn|pub|2322-2337|def deep_merge_dict(base: dict[str, Any], incoming: dict[...|
|`find_vscode_settings_source`|fn|pub|2338-2349|def find_vscode_settings_source() -> Optional[Path]|
|`build_prompt_recommendations`|fn|pub|2350-2364|def build_prompt_recommendations(prompts_dir: Path) -> di...|
|`ensure_wrapped`|fn|pub|2365-2380|def ensure_wrapped(target: Path, project_base: Path, code...|
|`save_vscode_backup`|fn|pub|2381-2395|def save_vscode_backup(req_root: Path, settings_path: Pat...|
|`restore_vscode_settings`|fn|pub|2396-2411|def restore_vscode_settings(project_base: Path) -> None|
|`prune_empty_dirs`|fn|pub|2412-2429|def prune_empty_dirs(root: Path) -> None|
|`remove_generated_resources`|fn|pub|2430-2474|def remove_generated_resources(project_base: Path) -> None|
|`run_remove`|fn|pub|2475-2524|def run_remove(args: Namespace) -> None|
|`_validate_enable_static_check_command_executables`|fn|priv|2525-2528|def _validate_enable_static_check_command_executables(|
|`run`|fn|pub|2557-2756|def run(args: Namespace) -> None|
|`VERBOSE`|var|pub|2566||
|`DEBUG`|var|pub|2567||
|`PROMPT`|var|pub|3026||
|`_format_install_table`|fn|priv|3452-3454|def _format_install_table(|
|`_wrap_cell`|fn|priv|3491-3513|def _wrap_cell(value: str, width: int, allow_wrap: bool) ...|
|`_render_row`|fn|priv|3514-3542|def _render_row(provider: str, prompts: str, modules: str...|
|`_build_provider_modules_map`|fn|priv|3563-3602|def _build_provider_modules_map(provider_specs: list[str]...|
|`_colorize_table_border`|fn|priv|3603-3615|def _colorize_table_border(line: str) -> str|
|`SUPPORTED_EXTENSIONS`|var|pub|3630||
|`_collect_source_files`|fn|priv|3658-3715|def _collect_source_files(src_dirs: list[str], project_ba...|
|`_build_ascii_tree`|fn|priv|3716-3761|def _build_ascii_tree(paths: list[str]) -> str|
|`_emit`|fn|priv|3740-3742|def _emit(|
|`_format_files_structure_markdown`|fn|priv|3762-3776|def _format_files_structure_markdown(files: list[str], pr...|
|`_is_standalone_command`|fn|priv|3777-3795|def _is_standalone_command(args: Namespace) -> bool|
|`_is_project_scan_command`|fn|priv|3796-3819|def _is_project_scan_command(args: Namespace) -> bool|
|`_is_here_only_project_scan_command`|fn|priv|3820-3844|def _is_here_only_project_scan_command(args: Namespace) -...|
|`run_git_check`|fn|pub|3845-3876|def run_git_check(args: Namespace) -> None|
|`run_docs_check`|fn|pub|3877-3906|def run_docs_check(args: Namespace) -> None|
|`run_git_wt_name`|fn|pub|3907-3935|def run_git_wt_name(args: Namespace) -> None|
|`_worktree_path_exists_exact`|fn|priv|3936-3965|def _worktree_path_exists_exact(git_path: Path, target_pa...|
|`_rollback_worktree_create`|fn|priv|3966-4002|def _rollback_worktree_create(git_path: Path, wt_path: Pa...|
|`run_git_wt_create`|fn|pub|4003-4094|def run_git_wt_create(args: Namespace) -> None|
|`run_git_wt_delete`|fn|pub|4095-4172|def run_git_wt_delete(args: Namespace) -> None|
|`run_git_wt_exit`|fn|pub|4173-4189|def run_git_wt_exit(args: Namespace) -> None|
|`run_files_tokens`|fn|pub|4190-4212|def run_files_tokens(files: list[str]) -> None|
|`run_files_references`|fn|pub|4213-4229|def run_files_references(files: list[str]) -> None|
|`run_files_compress`|fn|pub|4230-4248|def run_files_compress(files: list[str], enable_line_numb...|
|`run_files_find`|fn|pub|4249-4277|def run_files_find(args_list: list[str], enable_line_numb...|
|`run_references`|fn|pub|4278-4295|def run_references(args: Namespace) -> None|
|`run_compress_cmd`|fn|pub|4296-4317|def run_compress_cmd(args: Namespace) -> None|
|`run_find`|fn|pub|4318-4347|def run_find(args: Namespace) -> None|
|`run_tokens`|fn|pub|4348-4375|def run_tokens(args: Namespace) -> None|
|`run_files_static_check_cmd`|fn|pub|4376-4445|def run_files_static_check_cmd(files: list[str], args: Na...|
|`run_project_static_check_cmd`|fn|pub|4446-4491|def run_project_static_check_cmd(args: Namespace) -> int|
|`_resolve_project_base`|fn|priv|4492-4512|def _resolve_project_base(args: Namespace) -> Path|
|`_resolve_project_src_dirs`|fn|priv|4513-4565|def _resolve_project_src_dirs(args: Namespace) -> tuple[P...|
|`main`|fn|pub|4566-4666|def main(argv: Optional[list[str]] = None) -> int|
|`VERBOSE`|var|pub|4590||
|`DEBUG`|var|pub|4591||
