...
- **SRS-087**: The implementation MUST preserve this behavior exactly: The resource generation described in the specific sections MUST only perform when the corresponding `--enable-<provider>` flag is active.
...
- **SRS-092**: The `.github/prompts` front matter MUST reference the agent (only `agent: req-<name>`) when `--prompts-use-agents` is enabled AND BOTH `--enable-github` AND `--install-prompts` are active; otherwise the full prompt body with metadata MUST be emitted.
...
- **SRS-106**: For each Markdown prompt, the CLI MUST generate `.opencode/command/req-<name>.md`; when `--prompts-use-agents` is enabled, the front matter MUST reference `agent: req-<name>`.
...
- **SRS-111**: For each Markdown prompt, the CLI MUST generate a file in `.claude/commands/req` with front matter (agent ref if `--prompts-use-agents` is enabled, optional model/tools/allowed-tools) when BOTH `--enable-claude` AND `--install-prompts` are active.
