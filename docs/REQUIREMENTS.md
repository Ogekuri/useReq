...
- **SRS-034**: The CLI MUST accept boolean flags `--enable-claude`, `--enable-codex`, `--enable-gemini`, `--enable-github`, `--enable-kiro`, `--enable-opencode`, `--install-prompts`, `--install-agents`, `--install-skills`, `--legacy`, and `--preserve-models`. Default state MUST be: `--install-prompts=false`, `--install-agents=false`, `--install-skills=false`. When a provider `--enable-<provider>` flag is omitted and no `--provider` spec targets it, the CLI MUST skip all resource creation for that provider. When `--install-prompts` is active, prompt/command artifact files MUST be generated for each enabled provider that produces them. When `--install-agents` is active, agent artifact files MUST be generated for each enabled provider that produces them. When `--install-skills` is active, skill artifact files MUST be generated for each enabled provider that produces them. When `--legacy` is active, the CLI MUST activate legacy mode for configuration loading. When `--preserve-models` is active in combination with `--update`, the CLI MUST preserve the existing `.req/models.json` file and the `--legacy` flag has no effect.
- **SRS-035**: During installation without `--update`, the CLI MUST require at least one enabled provider (via `--enable-<provider>` or `--provider`) and at least one artifact activation (via `--install-prompts`, `--install-agents`, `--install-skills`, or the artifacts component of `--provider`); otherwise it MUST print an English error and exit code 4.
- **SRS-036**: The implementation MUST preserve this behavior exactly: The ASCII installation summary table MUST include rows only for CLI targets whose prompts were installed during the current invocation.
...
- **SRS-043**: The `--upgrade` option MUST run `uv tool install <program_name> --force --from git+https://github.com/<owner>/<repository>.git`, where `<owner>/<repository>` is resolved from active git remotes, and MUST fail on non-zero exit.
...
- **SRS-064**: With `--update`, the CLI MUST load paths, persisted boolean flags, and persisted `providers` array from `.req/config.json`, then apply only CLI-provided flags and `--provider` specs as overrides; if provider/artifact activation is invalid, it MUST raise a config-invalid error.
...
- **SRS-066**: The implementation MUST preserve this behavior exactly: The command MUST copy the `.req/models.json` configuration file, replacing the pre-existing file if present, unless `--preserve-models` is active. When `--preserve-models` is NOT active: if `--legacy` is NOT active, it MUST copy `models.json` from package resources (`src/usereq/resources/common/models.json`); if `--legacy` is active and `models-legacy.json` file is successfully loaded (i.e. when it exists), it MUST copy `models-legacy.json` from package resources (`src/usereq/resources/common/models-legacy.json` This copy MUST take place after creating the `.req` directory and after saving `config.json`.
...
- **SRS-081**: The implementation MUST preserve this behavior exactly: The command MUST support `--enable-models` and `--enable-tools` options to include `model` and `tools` fields in generated files.
- **SRS-082**: The implementation MUST preserve this behavior exactly: With `--enable-models`, include `model: <valore>` if present in `config.json` of the resource.
- **SRS-083**: The implementation MUST preserve this behavior exactly: With `--enable-tools`, include `tools` derived from `usage_modes` if present in `config.json`.
...
- **SRS-277**: When `--provider` is used, the provider enablement and artifact activation it declares MUST be merged with any global `--enable-<provider>`, `--install-<artifact>`, `--enable-models`, `--enable-tools`, `--prompts-use-agents`, and `--legacy` flags; global flags apply to all providers and `--provider` overrides or extends per-provider scope.
...
- **SRS-281**: For each provider targeted by a `--provider` spec, `enable-models` and `enable-tools` options MUST override the global `--enable-models` and `--enable-tools` flags for that provider's resource generation only.
- **SRS-282**: For each provider targeted by a `--provider` spec, the `prompts-use-agents` option MUST override the global `--prompts-use-agents` flag for that provider's prompt/command generation only.
- **SRS-283**: For each provider targeted by a `--provider` spec, the `legacy` option MUST override the global `--legacy` flag for that provider's model/configuration loading only.

### 3.3 Provider Resource Generation
- **SRS-087**: The implementation MUST preserve this behavior exactly: The resource generation described in the specific sections MUST only perform when the corresponding provider is activated (via `--enable-<provider>` flag or a `--provider` spec targeting it).
- **SRS-089**: The CLI MUST create provider artifact directories only when the corresponding provider flag AND the corresponding artifact-type activation are both active: `.codex/prompts` when `--enable-codex` AND `--install-prompts`; `.codex/skills` when `--enable-codex` AND `--install-skills`; `.github/agents` when `--enable-github` AND `--install-agents`; `.github/prompts` when `--enable-github` AND `--install-prompts`; `.github/skills` when `--enable-github` AND `--install-skills`; `.gemini/commands` and `.gemini/commands/req` when `--enable-gemini` AND `--install-prompts`; `.gemini/skills` when `--enable-gemini` AND `--install-skills`; `.kiro/agents` when `--enable-kiro` AND `--install-agents`; `.kiro/prompts` when `--enable-kiro` AND `--install-prompts`; `.kiro/skills` when `--enable-kiro` AND `--install-skills`; `.claude/agents` when `--enable-claude` AND `--install-agents`; `.claude/commands` and `.claude/commands/req` when `--enable-claude` AND `--install-prompts`; `.claude/skills` when `--enable-claude` AND `--install-skills`; `.opencode/agent` when `--enable-opencode` AND `--install-agents`; `.opencode/command` when `--enable-opencode` AND `--install-prompts`; `.opencode/skill` when `--enable-opencode` AND `--install-skills`.
- **SRS-090**: For each Markdown prompt, the CLI MUST copy to `.codex/prompts/req-<name>.md` with token substitutions when BOTH `--enable-codex` AND `--install-prompts` are active.
- **SRS-091**: For each Markdown prompt, the CLI MUST create `.github/prompts/req-<name>.prompt.md` when BOTH `--enable-github` AND `--install-prompts` are active.
- **SRS-092**: The `.github/prompts` front matter MUST reference the agent (only `agent: req-<name>`) when `prompts-use-agents` is active for the GitHub provider (via global `--prompts-use-agents` flag or the `prompts-use-agents` option in a `--provider github:...:prompts-use-agents` spec) AND BOTH `--enable-github` AND `--install-prompts` are active; otherwise the full prompt body with metadata MUST be emitted.
- **SRS-093**: For each Markdown prompt, the CLI MUST generate `.github/agents/req-<name>.agent.md` with front matter including `name` when BOTH `--enable-github` AND `--install-agents` are active.
- **SRS-094**: For each enabled provider with active skills (`--install-skills` active), the CLI MUST create a `skills` directory under the provider root (or `skill` for OpenCode) and, for each Markdown prompt, the subdirectory `<skills_root>/req-<prompt_name>`: `.codex/skills/req-<prompt_name>` when `--enable-codex` AND `--install-skills`; `.github/skills/req-<prompt_name>` when `--enable-github` AND `--install-skills`; `.gemini/skills/req-<prompt_name>` when `--enable-gemini` AND `--install-skills`; `.kiro/skills/req-<prompt_name>` when `--enable-kiro` AND `--install-skills`; `.claude/skills/req-<prompt_name>` when `--enable-claude` AND `--install-skills`; `.opencode/skill/req-<prompt_name>` when `--enable-opencode` AND `--install-skills`.
...
- **SRS-096**: The CLI MUST create `.gemini/commands` and `.gemini/commands/req` when BOTH `--enable-gemini` AND `--install-prompts` are active.
- **SRS-097**: For each Markdown prompt, the CLI MUST generate a TOML file in `.gemini/commands/req` by converting Markdown and applying token substitutions when BOTH `--enable-gemini` AND `--install-prompts` are active.
- **SRS-098**: The CLI MUST create `.kiro/agents` when BOTH `--enable-kiro` AND `--install-agents` are active; MUST create `.kiro/prompts` when BOTH `--enable-kiro` AND `--install-prompts` are active.
- **SRS-099**: For each Markdown prompt, the CLI MUST copy to `.kiro/prompts/req-<name>.md` preserving front matter when BOTH `--enable-kiro` AND `--install-prompts` are active.
- **SRS-100**: For each Markdown prompt, the CLI MUST generate `.kiro/agents/req-<name>.json` using the agent template from `models.json` when BOTH `--enable-kiro` AND `--install-agents` are active.
...
- **SRS-104**: The CLI MUST create `.opencode/agent` when BOTH `--enable-opencode` AND `--install-agents` are active; MUST create `.opencode/command` when BOTH `--enable-opencode` AND `--install-prompts` are active.
- **SRS-105**: For each Markdown prompt, the CLI MUST generate `.opencode/agent/req-<name>.md` with front matter and `mode: all` when BOTH `--enable-opencode` AND `--install-agents` are active.
- **SRS-106**: For each Markdown prompt, the CLI MUST generate `.opencode/command/req-<name>.md`; when `prompts-use-agents` is active for the OpenCode provider (via global `--prompts-use-agents` or `--provider opencode:...:prompts-use-agents`), the front matter MUST reference `agent: req-<name>`.
- **SRS-107**: The CLI MUST create `.claude/agents` when BOTH `--enable-claude` AND `--install-agents` are active.
- **SRS-108**: For each Markdown prompt, the CLI MUST generate `.claude/agents/req-<name>.md` with token substitutions when BOTH `--enable-claude` AND `--install-agents` are active.
- **SRS-109**: In `.claude/agents`, the front matter MUST include `name` and `description`; `model`/`tools` MUST be included only if `--enable-models`/`--enable-tools` are active and the value is configured. This applies when BOTH `--enable-claude` AND `--install-agents` are active.
- **SRS-110**: The CLI MUST create `.claude/commands/req` when BOTH `--enable-claude` AND `--install-prompts` are active.
- **SRS-111**: For each Markdown prompt, the CLI MUST generate a file in `.claude/commands/req` with front matter (agent ref if `prompts-use-agents` is active for the Claude provider via global `--prompts-use-agents` or `--provider claude:...:prompts-use-agents`, optional model/tools/allowed-tools) when BOTH `--enable-claude` AND `--install-prompts` are active.

- **SRS-112**: The CLI MUST accept three artifact-type controls: `--install-prompts` (default: false), `--install-agents` (default: false), and `--install-skills` (default: false). These controls operate orthogonally to provider flags and are evaluated per provider at generation time: `--install-prompts` enables generation of prompt/command artifact files for each enabled provider; `--install-agents` enables generation of agent artifact files for each enabled provider; `--install-skills` enables generation of skill artifact files for each enabled provider. During normal installation (non-upgrade/remove/help), installation processing MUST start only when at least one artifact-type control is provided; if none is provided, the CLI MUST print an English error message and exit with exit code 4. The artifact-type controls apply to each enabled provider independently: if a provider does not produce a given artifact type (e.g., Gemini produces only prompts, not agents), the corresponding control has no visible effect for that provider.
...
- **SRS-114**: For each Markdown prompt, the CLI MUST generate `.claude/skills/req-<prompt_name>/SKILL.md` with YAML front matter (`name`, `description` from SRS-113, optional `model`/`tools`) and prompt body with token substitutions, using the `claude` configuration from `models.json`, when `--enable-claude` and `--install-skills` are active.
- **SRS-115**: For each Markdown prompt, the CLI MUST generate `.gemini/skills/req-<prompt_name>/SKILL.md` with YAML front matter (`name`, `description` from SRS-113, optional `model`/`tools`) and prompt body with token substitutions, using the `gemini` configuration from `models.json`, when `--enable-gemini` and `--install-skills` are active.
- **SRS-116**: For each Markdown prompt, the CLI MUST generate `.opencode/skill/req-<prompt_name>/SKILL.md` with YAML front matter (`name`, `description` from SRS-113, optional `model`/`tools`) and prompt body with token substitutions, using the `opencode` configuration from `models.json`, when `--enable-opencode` and `--install-skills` are active.
- **SRS-117**: For each Markdown prompt, the CLI MUST generate `.github/skills/req-<prompt_name>/SKILL.md` with YAML front matter (`name`, `description` from SRS-113, optional `model`/`tools`) and prompt body with token substitutions, using the `copilot` configuration from `models.json`, when `--enable-github` and `--install-skills` are active.
- **SRS-118**: For each Markdown prompt, the CLI MUST generate `.kiro/skills/req-<prompt_name>/SKILL.md` with YAML front matter (`name`, `description` from SRS-113, optional `model`/`tools`) and prompt body with token substitutions, using the `kiro` configuration from `models.json`, when `--enable-kiro` and `--install-skills` are active.
...
- **SRS-287**: The project MUST include unit tests in `tests/test_cli.py` verifying that per-provider `enable-models`, `enable-tools`, `prompts-use-agents`, and `legacy` options from `--provider` specs override the corresponding global flags for the targeted provider only.
