...
- **SRS-242**: The implementation MUST provide a `StaticCheckPylance` class derived from `StaticCheckBase` that invokes `pyright` per resolved file; when `fail_only` mode is inactive, MUST emit `# Static-Check(Pylance): <filename> [OPTIONS]`, emit `Result: OK` or `Result: FAIL\nEvidence:\n<output>`, and append exactly one trailing blank line after each per-file block; when `fail_only` mode is active, a passing file MUST produce no output and a failing file MUST emit header, `Result: FAIL\nEvidence:\n<output>`, and one trailing blank line.
- **SRS-243**: The implementation MUST provide a `StaticCheckRuff` class derived from `StaticCheckBase` that invokes `ruff check` per resolved file; when `fail_only` mode is inactive, MUST emit `# Static-Check(Ruff): <filename> [OPTIONS]`, emit `Result: OK` or `Result: FAIL\nEvidence:\n<output>`, and append exactly one trailing blank line after each per-file block; when `fail_only` mode is active, a passing file MUST produce no output and a failing file MUST emit header, `Result: FAIL\nEvidence:\n<output>`, and one trailing blank line.
