...
- **SRS-291**: The `Modules Installed` column MUST render one line per active `--provider` artifact as `artifact` when no options are active, or `artifact:options` when options are active, where `artifact` is `prompts` or `agents`.
...
- **SRS-089**: The CLI MUST create provider artifact directories only when the corresponding provider is targeted by a `--provider` spec with the corresponding artifact type: `.codex/prompts` for codex `prompts`; `.github/agents` and `.github/prompts`; `.gemini/commands` and `.gemini/commands/req`; `.kiro/agents` and `.kiro/prompts`; `.claude/agents` and `.claude/commands` plus `.claude/commands/req`; `.opencode/agent` and `.opencode/command`.
...
- **SRS-094**: ~~REMOVED~~ (Skills artifact directory generation is no longer supported by provider specifications.)
...
- **SRS-114**: ~~REMOVED~~ (Generation of `.claude/skills/.../SKILL.md` is no longer supported.)
- **SRS-115**: ~~REMOVED~~ (Generation of `.gemini/skills/.../SKILL.md` is no longer supported.)
- **SRS-116**: ~~REMOVED~~ (Generation of `.opencode/skill/.../SKILL.md` is no longer supported.)
- **SRS-117**: ~~REMOVED~~ (Generation of `.github/skills/.../SKILL.md` is no longer supported.)
- **SRS-118**: ~~REMOVED~~ (Generation of `.kiro/skills/.../SKILL.md` is no longer supported.)
...
- **SRS-325**: The provider-to-directory mapping for `--git-wt-create` MUST be: claude -> `.claude/commands`, `.claude/agents`; gemini -> `.gemini/commands`; github -> `.github/prompts`, `.github/agents`; codex -> `.codex/prompts`; kiro -> `.kiro/prompts`, `.kiro/agents`; opencode -> `.opencode/agent`, `.opencode/command`.
