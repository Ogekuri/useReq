...
- **SRS-029**: When `req` is invoked with `--ver` or `--version`, the command MUST print the version number, and a startup bright-red newer-version warning MAY precede it when online release-check detects a newer release.
