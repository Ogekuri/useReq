...
- **SRS-349**: MUST persist a 300-second idle-delay after a successful startup HTTP release-check; when the fetched version is newer than the installed version, it MUST print a bright-green stderr message `New version available: installed <installed_version>, latest <latest_version>.`.
- **SRS-350**: MUST print a bright-red stderr diagnostic for HTTP release-check failures; on HTTP 429 or on HTTP 403 with response message containing `API rate limit exceeded`, it MUST persist a 3600-second idle-delay.
