...
def _normalize_modules(modules: str | Iterable[str]) -> list[str]:
    """Returns a list of modules from either a string or an iterable."""
...
def _run_pdoc(command: list[str], *, env: dict[str, str], cwd: Path) -> subprocess.CompletedProcess:
    """Runs pdoc and captures output for error handling."""
...
def generate_pdoc_docs(
    ...
    *,
    ...
    """Generates or updates pdoc documentation in the target output directory.

    Args:
        output_dir: Directory where HTML documentation will be written.
        modules: Importable modules or packages to document.
        all_submodules: When True, recurse into submodules even if not exported via __all__.
    """
