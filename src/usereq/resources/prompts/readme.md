...
5. **CRITICAL**: Stage & commit
   ...
   - Confirm the repo is clean with `git status --porcelain`. If it is NOT empty, override the final line with EXACTLY "WARNING: README request completed with unclean git repository!\x07".
6. **CRITICAL**: Merge Conflict Management
   ...
   - If the merge fails or results in conflicts, do NOT remove the worktree directory and override the final line with EXACTLY "WARNING: README request completed with merge conflicting!\x07".
7. Present results
