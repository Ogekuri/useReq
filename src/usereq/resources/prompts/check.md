...
1. Run the test suite to verify the current state. Do not modify the source code or tests. Record the test results (`OK`/`FAIL`) to be used as evidence for the final analysis report.
...
5. OUTPUT exactly "Check completed!" as the FINAL line (plain text, no markdown/code block, have no trailing spaces), and STOP.
