...
3. Generate and apply the **Software Requirements Specification Update** to change requirements
   ...
   - APPLY the **Software Requirements Specification Update** to `%%DOC_PATH%%/REQUIREMENTS.md`, following its formatting and guidelines; the resulting `%%DOC_PATH%%/REQUIREMENTS.md` MUST be in English. Do NOT introduce any additional edits beyond what the **Software Requirements Specification Update** describes.
...
9.  Present results
   - PRINT, in the response, the results in fixed sections in this exact order: "Workflow Coverage", "Execution Units Delta", "Communication Edges Delta", "Call-Trace Evidence", with concise evidence pointers (path + symbol + line range). The final line of the output must be EXACTLY "Change request completed!".
