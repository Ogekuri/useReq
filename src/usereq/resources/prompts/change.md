...
3. Produce a comprehensive change proposal describing the edits to requirements needed to implement the changes described by the [User Request](#users-request). This proposal must account for the original User Request and all subsequent changes and adjustments for %%REQ_DOC%%. 
...
5. PRINT in the response presenting the detailed **requirements changes** (only requirements, full detailed content needed for implementation, do not summarize).
6. Apply the **requirements changes** to %%REQ_DOC%%, following its formatting, language, and guidelines from the template at `.req/templates/requirements.md`. Do NOT introduce any additional edits beyond what the **requirements changes** describes.
...
9.  If a migration/compatibility need is discovered but not specified in requirements, propose a requirements update describing it, then OUTPUT exactly "Change request FAILED!" as the FINAL line, and STOP.
...
13. Implement the **source code changes** in the source code (creating new files/directories if necessary). You may make minimal mechanical adjustments needed to fit the actual codebase (file paths, symbol names), but you MUST NOT add new features or scope beyond the **source code changes**.  
...
16. Run the updated test suite. 
   ...
   - Fix the source code to pass valid tests. After fixing, re-run the relevant tests to confirm they pass. Attempt to fix up to 2 times then, if they fail again, report the failure, then OUTPUT exactly "Change request FAILED!" as the FINAL line, and STOP.
   - Limitations: Do not introduce new features or change the architecture logic during this fix phase. If a fix requires substantial refactoring or requirements changes, report the failure, then OUTPUT exactly "Change request FAILED!" as the FINAL line, and STOP.
...
20. Confirm the repo is clean with `git status --porcelain`, If NOT empty OUTPUT exactly "Change request FAILED!" as the FINAL line, and STOP.
21. OUTPUT exactly "Change request completed!" as the FINAL line, and STOP.
