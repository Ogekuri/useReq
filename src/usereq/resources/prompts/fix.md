...
8.  Run the updated test suite. 
   ...
   - Fix the source code to pass valid tests. Execute a strict fix loop: 1) Analyze the failure, 2) Fix code, 3) Re-run tests. Repeat this loop up to 2 times. If tests still fail after the second attempt, report the failure, OUTPUT exactly "Defect fix FAILED!", revert changes executing `git checkout .` and `git clean -fd`, and then terminate the execution.
