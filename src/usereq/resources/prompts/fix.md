...
11. Run the updated test suite. 
   ...
   - Fix the source code to pass valid tests. After fixing, re-run the relevant tests to confirm they pass. Attempt to fix up to 2 times then, if they fail again, report the failure, then OUTPUT exactly "Defect fix FAILED!" as the FINAL line, and STOP
   - Limitations: Do not introduce new features or change the architecture logic during this fix phase; if a fix requires substantial refactoring or requirements changes, report the failure, then OUTPUT exactly "Defect fix FAILED!" as the FINAL line, and STOP.
...
15. Confirm the repo is clean with `git status --porcelain`, If NOT empty OUTPUT exactly "Defect fix FAILED!" as the FINAL line, and STOP.
16. OUTPUT exactly "Defect fixed!" as the FINAL line, and STOP.
