...
5. Update `%%DOC_PATH%%/WORKFLOW.md` via targeted edits using the canonical WORKFLOW.md contract (same terminology, same schema, same call-trace rules).
...
7. **CRITICAL**: Stage & commit
      ...
      - Commit a structured commit message with: `git commit -m "cover(<COMPONENT>)<BRAKING>: <DESCRIPTION> [useReq]"`
