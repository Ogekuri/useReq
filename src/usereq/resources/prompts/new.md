...
5. Do not change the intent of existing requirements unless the new feature logically requires it. You may make minimal edits for consistency (references, numbering, glossary) as long as you explicitly list them.
...
8. GENERATE a detailed **Comprehensive Technical Implementation Report** documenting the exact modifications to the source code that will cover all `FAIL` requirements and the [User Request](#users-request). The **Comprehensive Technical Implementation Report** MUST only code logic, full detailed content needed for implementation, do not summarize.
...
17. **CRITICAL**: Stage & commit
   ...
   - Commit a structured commit message with: `git commit -m "change(useReq): <DESCRIPTION> [<DATE>]"`
      ...
      - Generate `<DESCRIPTION>` as clear and concise description of changes made on requirements and source code, using English language.
18. Confirm the repo is clean with `git status --porcelain`, If NOT empty OUTPUT exactly "New implementation FAILED!", and then terminate the execution.
19. OUTPUT exactly "New implementation completed!".
