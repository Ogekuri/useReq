...
- If maintaining compatibility would require migrations/auto-upgrades conversion logic, report the conflict instead of implementing and STOP.
...
16. Run the updated test suite. 
   ...
   - Fix the source code to pass valid tests. After fixing, re-run the relevant tests to confirm they pass. Attempt to fix up to 2 times then, if they fail again, report the failure, then OUTPUT exactly "New implementation FAILED!" as the FINAL line, and STOP.
   - Limitations: Do not introduce new features or change the architecture logic during this fix phase; if a fix requires substantial refactoring or requirements changes, report the failure, then OUTPUT exactly "New implementation FAILED!" as the FINAL line, and STOP.
...
20. Confirm the repo is clean with `git status --porcelain`, If NOT empty OUTPUT exactly "New implementation FAILED!" as the FINAL line, and STOP.
21. OUTPUT exactly "New implementation completed!" as the FINAL line, and STOP.
