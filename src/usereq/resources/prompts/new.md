...
3. Produce a comprehensive change proposal describing the edits to requirements needed to implement the feature(s) described by the [User Request](#users-request). This proposal must account for the original request and all subsequent adjustments logged in that file.  
...
20. Confirm the repo is clean with `git status --porcelain`, If NOT empty OUTPUT exactly "New implementation FAILED!" as the FINAL line (plain text, no markdown/code block, have no trailing spaces), and STOP.
21. OUTPUT exactly "New implementation completed!" as the FINAL line (plain text, no markdown/code block, have no trailing spaces), and STOP.
