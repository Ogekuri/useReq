...
## Steps
Render the **Global Roadmap** (markdown checklist `1..14`) at the start of your execution, then execute it step by step without pausing:
...
5. Apply the **Software Requirements Specification Update** to %%REQ_DOC%%, following its formatting, language, and guidelines from the template at `.req/templates/requirements.md`. Do NOT introduce any additional edits beyond what the **Software Requirements Specification Update** describes.
...
10. Run the updated test suite. 
   ...
   - Fix the source code to pass valid tests. Execute a strict fix loop: 1) Analyze the failure, 2) Fix code, 3) Re-run tests. Repeat this loop up to 2 times. If tests still fail after the second attempt, report the failure, OUTPUT exactly "New implementation FAILED!", revert changes executing `git checkout .` and `git clean -fd`, and then terminate the execution.
...
12. **CRITICAL**: Stage & commit
   ...
   - Commit a structured commit message with: `git commit -m "new(useReq): <DESCRIPTION> [<DATE>]"`
...
13. Confirm the repo is clean with `git status --porcelain`, If NOT empty OUTPUT exactly "New implementation FAILED!", and then terminate the execution.
