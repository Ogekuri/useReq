---
description: "Reorganize, update, and renumber the requirement draft based on source code analysis"
...
2. Read the requirements source document %%REQ_DOC%% and extract a complete, explicit list of atomic requirements.
...
4. Verify full coverage of the input requirements after reorganization.
   ...
   - If any requirement was rewritten for clarity, you MUST keep a traceable reference to its "Original ID" next to it (e.g., "(from SRC-012)"), and you MUST ensure the rewrite is meaning-preserving.
   - If any requirement is missing or duplicated, you MUST fix the structure before proceeding.
5. Add all information necessary to ensure each requirement is placed in the correct section/subsection.
   - For each section/subsection you created, add a short, unambiguous "Scope/Grouping rule" text stating what belongs there.
   - For each requirement that is context-poor (e.g., missing actor, trigger, scope, or object), add minimal clarifying context so its placement is justified.
   - Do not invent domain facts. Any inference MUST be explicitly labeled as either:
     - "Inferred from requirements text", or
     - "Inferred from code" (and later supported with evidence during PRINT).
...
7. Create or update a Markdown file with the requirements draft at `docs/requirements_DRAFT.md`.
...
8. Re-number all requirements in `docs/requirements_DRAFT.md` and preserve all cross-references.
   ...
   - If renumbering and reference-updating cannot be performed safely with simple text operations, you MUST implement and execute a Python renumbering utility.
     ...
     - The program MUST only read and write `docs/requirements_DRAFT.md` and MUST NOT write anywhere else.
     - The program MUST produce (at least in PRINT output) the mapping from old requirement IDs to new requirement IDs.
9. Re-read `docs/requirements_DRAFT.md` and cross-reference with the source code.
   - Verify that the draft **accurately reflects the actual code behavior** (True State).
...
10. Re-read `%%REQ_PATH%%/requirements_DRAFT.md` and cross-reference with the source code.
   ...
   - If the code contains obvious bugs or partial implementations, ensure the requirement draft explicitly notes these limitations.
   - Report `OK` if the draft accurately describes the code (even if the code is buggy). Report `FAIL` only if the draft makes assertions that are not present or contradicted by the source code.
11. Record the `OK`/`FAIL` statuses generated during the re-read steps along with concise evidence for any `FAIL`, including file paths and line numbers when relevant.
