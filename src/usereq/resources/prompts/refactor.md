...
3. Generate and implement the **Comprehensive Technical Implementation Report** to implement the refactor
   ...
   - From %%TEST_PATH%%, locate and read only the unit tests that are relevant to the affected requirement IDs and touched modules (via targeted search), and plan the necessary refactoring/additions to cover performance-critical paths and include these details in the **Comprehensive Technical Implementation Report**.
...
8. Present results
   - PRINT, in the response, the results in fixed sections in this exact order: "Workflow Coverage", "Execution Units Delta", "Communication Edges Delta", "Call-Trace Evidence", with concise evidence pointers (path + symbol + line range). The final line of the output must be EXACTLY "Refactor completed!".
