...
4. **CRITICAL**: Stage & commit
   ...
   - Confirm the repo is clean with `git status --porcelain`. If it is NOT empty, override the final line with EXACTLY "WARNING: References request completed with unclean git repository!\\x07".
5. **CRITICAL**: Merge Conflict Management
   ...
   - If the merge fails or results in conflicts, do NOT remove the worktree directory and override the final line with EXACTLY "WARNING: References request completed with merge conflicting!\\x07".
6. Present results
